...
def game_content_bundle():
    ...
    with Connect(in_memory=True) as c_m:
        bundle_download = BundleDownload(c_m)
        effective_bundle_version = bundle_version
        bundle_download.set_client_info(
            app_version, effective_bundle_version, device_id)
        ordered_results = bundle_download.get_bundle_list()
        if (not bundle_version or bundle_version == '0.0.0') and len(ordered_results) > 1:
            ordered_results = ordered_results[:1]
        current_app.logger.info(
            'content_bundle request app=%s bundle=%s effective_bundle=%s device=%s results=%s versions=%s',
            app_version,
            bundle_version,
            effective_bundle_version,
            device_id,
            len(ordered_results),
            [item.get('contentBundleVersion') for item in ordered_results],
        )
    ...
    with Connect(in_memory=True) as c_m:
        with Connect() as c:
            x = DownloadList(c_m, UserOnline(c, user_id),
                             request.headers.get('AppVersion'))
...
@bp.route('/unlock/me/<path:unlock_id>', methods=['POST'])
@auth_required(request)
@arc_try
def unlock_me(user_id, unlock_id):
    current_app.logger.info(f"unlock_me: user_id={user_id}, unlock_id={unlock_id}")
    with Connect() as c:
        if unlock_id == 'konzetsu_saya':
            item = ItemCharacter(c)
            item.set_id('97')
            item.user_claim_item(UserOnline(c, user_id))
            if Config.CHARACTER_FULL_UNLOCK:
                c.execute('''insert or ignore into user_char_full values(?,?,?,?,?,?,0)''',
                          (user_id, 97, 20, 10000, 0, 0))
            current_app.logger.info(
                f"Successfully unlocked Saya (Absolution) (ID 97) for user {user_id}")
        elif unlock_id.startswith('awaken_'):
            char_name = unlock_id[7:]
            c.execute('select character_id from character where lower(name) = ?', (char_name.lower(),))
            row = c.fetchone()
            if row:
                char_id = row[0]
                item = ItemCharacter(c)
                item.set_id(str(char_id))
                item.user_claim_item(UserOnline(c, user_id))
                c.execute('update user_char set is_uncapped = 1, is_uncapped_override = 0 where user_id = ? and character_id = ?', (user_id, char_id))
                current_app.logger.info(f"Successfully awakened character {char_name} (ID {char_id}) for user {user_id}")
            else:
                current_app.logger.warning(f"unlock_me: character {char_name} not found")
        else:
            current_app.logger.info(f"unlock_me: unhandled generic unlock_id = {unlock_id}")
    return success_return({})
