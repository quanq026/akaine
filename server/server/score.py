...
def canonical_score_song_id(song_id):
    return 'testify' if song_id == 'testify1' else song_id


def score_rows_for_app_version(rows, app_version):
    if app_version == '7.0.255':
        return rows
    return [
        dict(row, song_id='testify1')
        if row.get('song_id') == 'testify'
        else row
        for row in rows
    ]
...
def song_score_post(user_id):
    with Connect() as c:
        ...
        x.rank_bonus = request.form.get('rank_bonus', type=int)
        x.maya_gauge = request.form.get('maya_gauge', type=int)
        x.nextstage_bonus = request.form.get('nextstage_bonus', type=int)
        x.room_code = request.form.get('room_code')
        x.room_total_score = request.form.get('room_total_score', type=int)
        x.room_total_players = request.form.get('room_total_players', type=int)
        if not x.is_valid:
            print(f'AKAINE_SCORE_DIAG rejected reason={x.validation_failure}')
            raise InputError('Invalid score.', 107)
        x.song.song_id = canonical_score_song_id(x.song.song_id)
        x.upload_score()
        # room_code???
...
def song_score_top(user_id):
    with Connect() as c:
        ...
        rank_list.song.set_chart(canonical_score_song_id(request.args.get(
            'song_id')), request.args.get('difficulty'))
        rank_list.select_top()
        return success_return(score_rows_for_app_version(
            rank_list.to_dict_list(), request.headers.get('AppVersion')
        ))
