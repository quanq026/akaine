...
def room_create(user_id):
    ...
    with Connect() as c:
        ...
        logging.info(
            "ROOM_CREATE_REQUEST: protocolVersion=%s clientSongMap_entries=%s",
            request_payload.get('protocolVersion'),
            len(request_payload.get('clientSongMap', {})))
        x = RemoteMultiPlayer()
        ...
        logging.info(
            "ROOM_CREATE_RESPONSE: roomCode=%s roomId=%s playerId=%s unlockBytes=%s endPoint=%s port=%s",
            r.get('roomCode'),
            r.get('roomId'),
            r.get('playerId'),
            len(x.room.song_unlock),
            r.get('endPoint'),
            r.get('port'))
