...
def header_check(request) -> ArcError:
    ...
    expected_bundle_version = BundleParser.max_bundle_version.get(headers.get('AppVersion', ''))
    if request.method == 'GET' and 'ContentBundle' in headers and expected_bundle_version and headers['ContentBundle'] != expected_bundle_version:
        current_app.logger.info(
            'non-blocking content bundle mismatch app=%s client=%s expected=%s path=%s',
            headers.get('AppVersion', ''),
            headers.get('ContentBundle', ''),
