...
class WorldSkillMixin:
    ...
    def _skill_nonoka_uncap(self) -> None:
        ...
        self.character_bonus_progress_normalized = self.user_play.rank_bonus * 0.1 * self.progress_normalized
        self.user.current_map.reclimb(self.final_progress)

    def _skill_vita_arc(self) -> None:
        x = self.user_play.near_count // 2
        over = self.character_used.overdrive.get_value(self.character_used.level)
...
class BaseWorldPlay(WorldSkillMixin):
    ...
    def to_dict(self) -> dict:
        ...
        if arcmap.is_linkplay_allowed:
            r['linkplay_boost'] = self.linkplay_boost
            r['progress_before_linkplay_boost'] = self.progress_before_linkplay_boost
            r['progress_linkplay_boost_amount'] = self.progress_before_linkplay_boost * (self.linkplay_boost - 1) * self.step_times
