...
SONG_METADATA_PATH = os.path.join(os.path.dirname(Constant.SONG_FILE_FOLDER_PATH.rstrip('/\\')), 'song_metadata.json')
REMOTE70_SONG_METADATA_PATH = os.path.join(
    os.path.dirname(Constant.SONG_FILE_FOLDER_PATH.rstrip('/\\')),
    'remote70', 'song_metadata.json')
song_metadata = {}
for metadata_path in (SONG_METADATA_PATH, REMOTE70_SONG_METADATA_PATH):
    if not os.path.isfile(metadata_path):
        continue
    try:
        with open(metadata_path, 'r', encoding='utf-8') as f:
            song_metadata.update(json.load(f))
    except Exception:
        pass
...
def get_song_file_md5(song_id: str, file_name: str) -> str:
    if song_id in song_metadata and file_name in song_metadata[song_id].get("hashes", {}):
        return song_metadata[song_id]["hashes"][file_name]
...
def _asset_signing_secret() -> str:
    return Constant.ASSET_SIGNING_SECRET or os.environ.get('ASSET_SIGNING_SECRET', '')


def is_asset_signing_enabled(song_id: str) -> bool:
    if not Constant.ASSET_SIGNING_ENABLED:
        return False
    if not Constant.ASSET_SIGNED_PREFIX:
        return False
    if not _asset_signing_secret():
        return False
    if Constant.ASSET_SIGNING_PROTECT_ALL_SONGS:
        return True
    return song_id in set(Constant.ASSET_SIGNING_SONG_IDS or [])


def get_asset_signed_ttl(file_name: str) -> int:
    audio_files = ('base.ogg', '3.ogg', '4.ogg', 'video.mp4',
                   'video_audio.ogg', 'video_720.mp4', 'video_1080.mp4')
    if file_name in audio_files:
        return int(Constant.ASSET_SIGNED_TTL_AUDIO)
    return int(Constant.ASSET_SIGNED_TTL_CHART)


def build_signed_asset_url(song_id: str, file_name: str, user_id: int, now: int = None, nonce: str = None) -> str:
    created_at = int(time()) if now is None else int(now)
    expires_at = created_at + get_asset_signed_ttl(file_name)
    nonce = nonce or secrets.token_urlsafe(16)

    parsed = urlsplit(Constant.ASSET_SIGNED_PREFIX)
    path_prefix = parsed.path.rstrip('/')
    path = f'{path_prefix}/{quote(song_id, safe="")}/{quote(file_name, safe="")}'
    user_id = str(user_id)
    payload = '\n'.join([
        'GET',
        path,
...
class UserDownload:
    ...
    def __init__(self, c_m=None, user=None, download_link_prefix=None) -> None:
        self.c_m = c_m
        self.user = user
        self.download_link_prefix = (Constant.DOWNLOAD_LINK_PREFIX
                                     if download_link_prefix is None
                                     else download_link_prefix)
    ...
    def url(self) -> str:
        ...
        if is_asset_signing_enabled(self.song_id):
            return build_signed_asset_url(self.song_id, self.file_name, self.user.user_id)
        if self.download_link_prefix:
            prefix = self.download_link_prefix
            if prefix[-1] != '/':
                prefix += '/'
...
class DownloadList(UserDownload):
    ...
    def __init__(self, c_m=None, user=None, app_version=None) -> None:
        download_link_prefix = (
            Config.DOWNLOAD_LINK_PREFIX_7_0_255
            if app_version == '7.0.255'
            else Constant.DOWNLOAD_LINK_PREFIX
        )
        super().__init__(c_m, user, download_link_prefix)
    ...
    def get_one_song_file_names(song_id: str) -> list:
        ...
        if song_id in song_metadata:
            return song_metadata[song_id].get("files", [])
        r = []
        path = os.path.join(Constant.SONG_FILE_FOLDER_PATH, song_id)
    ...
    def add_one_song(self, song_id: str) -> None:
        ...
        for i in self.get_one_song_file_names(song_id):
            x = UserDownload(self.c_m, self.user, self.download_link_prefix)
            ...
            if i == 'base.ogg':
                ...
                if self.url_flag:
                    re['audio']["url"] = x.url
            elif len(i) == 5 and i[0] in '01234' and i[1:] == '.ogg':
                if 'audio' not in re:
                    re['audio'] = {}

                if self.url_flag:
                    re['audio'][i[0]] = {"checksum": x.hash, "url": x.url}
                else:
                    re['audio'][i[0]] = {"checksum": x.hash}
            elif i.lower().endswith('.wav') or i in ('video.mp4', 'video_audio.ogg', 'video_720.mp4', 'video_1080.mp4'):
    ...
    def get_all_song_ids() -> list:
        '''获取全歌曲文件夹列表，有lru缓存'''
        if song_metadata:
            return list(song_metadata.keys())
        if os.path.isdir(Constant.SONG_FILE_FOLDER_PATH):
    ...
    def add_songs(self, song_ids: list = None) -> None:
        ...
        if not self.song_ids:
            ...
            for i in self.song_ids:
                if i in song_metadata or os.path.isdir(os.path.join(Constant.SONG_FILE_FOLDER_PATH, i)):
                    self.add_one_song(i)
