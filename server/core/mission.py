...
# Arcaea 6.15 aborts when the login payload has an empty mission-state map.
# These are the exact intro states emitted by a known-good 6.15 client cache.
MISSION_STATE_BOOTSTRAP_IDS = (
...
class UserMissionList:
    ...
    def ensure_bootstrap_missions(self) -> bool:
        """Seed only a completely empty user with safe claimed intro states."""
        self.c.execute('''select exists(select 1 from user_mission where user_id=?)''',
                       (self.user.user_id,))
        ...
        self.c.executemany(
            ...
            [(self.user.user_id, mission_id) for mission_id in MISSION_STATE_BOOTSTRAP_IDS],
        )
        return True

    def select_all(self):
        self.missions = []
        self.ensure_bootstrap_missions()
